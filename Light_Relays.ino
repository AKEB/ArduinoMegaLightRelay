...
void loop() {
	...
	if (currentMillis < last_millis_water) last_millis_water = 0;
	if (currentMillis - last_millis_water > 500) {
		// Проверяем сеть
		check_net();
		
		last_millis_water = currentMillis;
	...
	if (currentMillis < last_millis_btn) last_millis_btn = 0;
	if (currentMillis - last_millis_btn > 500) {
		// Проверяем сеть
		check_net();
	...
	if (currentMillis < last_millis_reconnect) last_millis_reconnect = 0;
	if (currentMillis - last_millis_reconnect > 5000) {
